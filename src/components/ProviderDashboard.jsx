...
export default function ProviderDashboard({
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900/90 rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-slate-200/90 dark:border-white/10 shadow-clean-card relative overflow-hidden">
        {/* Decorative ambient gradient */}
        <div className="absolute top-0 right-0 w-80 h-80 bg-emerald-500/5 rounded-full blur-3xl pointer-events-none -z-0" />

        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 sm:gap-6 pb-5 sm:pb-6 border-b border-slate-100 dark:border-white/10">
          {/* Persona details */}
          {currentProvider && (
            <div className="flex items-start sm:items-center gap-3 sm:gap-4">
              <div className="relative shrink-0">
                <img
                  ...
                  className="w-14 h-14 sm:w-20 sm:h-20 rounded-xl sm:rounded-2xl object-cover border-2 border-[#1E3A2B] dark:border-emerald-500 shadow-md"
                />
                <span
                  className={`absolute -bottom-1 -right-1 w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full border-2 border-white dark:border-slate-900 ${
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] sm:text-[10px] font-extrabold uppercase tracking-wider px-2 py-0.5 rounded-full bg-[#EBF5F0] text-[#1E3A2B] dark:bg-emerald-950/60 dark:text-emerald-300 border border-emerald-300/40 shadow-xs">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-1 text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 font-medium">
                  <div className="flex items-center text-amber-500 font-bold">
                    <Star className="w-3 h-3 sm:w-3.5 sm:h-3.5 fill-amber-400 text-amber-400 mr-1" />
                    {currentProvider.rating?.toFixed(1)}
                    <span className="text-slate-400 font-normal ml-1">
                      ({currentProvider.reviewsCount})
                    </span>
                  </div>
                  <span>•</span>
                  <span className="text-slate-900 dark:text-white font-bold">
                    ৳{currentProvider.basePrice}/hr
                  </span>
                  <span>•</span>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3">
            ...
            <button
              ...
              className={`px-3.5 sm:px-4 py-2 rounded-full text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer shadow-xs ${
                isOnline
        ...
        {/* ---------------------------------------------------- */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-4 py-4 sm:py-6 border-b border-slate-100 dark:border-white/10">
        ...
        {/* Sub-Navigation Tabs (Touch-Scrollable on Mobile) */}
        <div className="flex overflow-x-auto no-scrollbar items-center gap-2 pt-4 sm:pt-6 pb-1">
          <button
            onClick={() => setProviderTab('queue')}
            className={`flex items-center gap-2 px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all relative cursor-pointer shrink-0 whitespace-nowrap ${
              providerTab === 'queue'
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all cursor-pointer shrink-0 whitespace-nowrap ${
